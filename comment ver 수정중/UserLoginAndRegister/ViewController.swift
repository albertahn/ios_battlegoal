...
class ViewController: UIViewController {
    ...
    override func viewDidAppear(animated: Bool) {
       ...
       // request.predicate = pred
        ...
        let commentUrl = NSURL(string: "http://mobile.tanggoal.com/comment/get_course_comments/1")
        ...
        let task = NSURLSession.sharedSession().dataTaskWithRequest(commentRequest) {
            ...
            if let parseJSON = json {
                var comment = parseJSON["comment_text"] as? String
                var membersindex = parseJSON["members_index"] as? String
                var likes = parseJSON["likes"] as? String
                var replynum = parseJSON["reply_num"] as? String
